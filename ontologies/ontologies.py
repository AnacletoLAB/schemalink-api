...
async def refresh_all(force: bool = False) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        new_metadata = await service.fetch_ontology_metadata_only()
        
        # Load custom ontologies metadata
        ...
        # Combine OLS4 and custom metadata for comparison
        combined_new_metadata = new_metadata + custom_metadata

        current_cached = await cache.load_as_dict()
        current_metadata = []
        for _, o in current_cached.items():
        ...
        current_hash = hashlib.md5(
            json.dumps(sorted(current_metadata, key=lambda x: x["id"]), sort_keys=True).encode()
        ).hexdigest()
        new_hash = hashlib.md5(
            json.dumps(sorted(combined_new_metadata, key=lambda x: x["id"]), sort_keys=True).encode()
        ).hexdigest()
 
        if current_hash == new_hash:
            logger.info("✅ Metadata hashes match - no changes detected")
        else:
            logger.info("🔄 Metadata hashes differ - changes detected")

        # Force refresh if: forced flag is True, cache is empty, OR metadata changed
        should_refresh = force or len(current_cached) == 0 or current_hash != new_hash
        
        if should_refresh:
            if force:
                logger.info("🔨 Force refresh requested - performing full refresh")
            elif len(current_cached) == 0:
                logger.info("🆕 Cache is empty, forcing full refresh")
            else:
                logger.info("🔄 Metadata changed, performing full refresh")
